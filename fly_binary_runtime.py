"""Autonomous OlympTrade DEMO runtime for the repository binary strategy.

Consumes real OlympTrade ticks, builds 5-minute OHLC candles, waits for a
complete 30-candle context, applies the strategy, and places only DEMO orders
when the strategy returns UP/DOWN and payout is >= 80%.
"""
...
from olymptrade_demo_v2 import MIN_PAYOUT, PAIRS, TRADE_SECONDS, OlympTradeDemoGateway
...
async def main() -> None:
    ...
    async def on_candle(candle):
        ...
        if decision.side not in {"UP", "DOWN"}:
            return
        result = await gateway.place_demo_order(pair, STAKE, decision.side.lower())
